...
class Bot {
    ...
    async play(msg) {
        ...
        if (musicURL.includes("playlist")) {
            ...
            await this.search(msg);
        } else {
            await this.dispatch(msg, musicURL, false);
        }
    }
    async search(msg) {
        const musicTitle = msg.content.includes(`${prefix}p`) ? await play.search(msg.content.replace(`${prefix}p`, '').trim(), { limit: 1 }) : await play.search(msg.content.replace(`${prefix}s`, '').trim(), { limit: 1 });;
    ...
    async lyrics(msg) {
        try {
            const lyric = await lyrics.search(this.queue[msg.guildId][0].name);
            msg.channel.send({embeds: [new EmbedBuilder()
                    .setColor('#3498DB')
                    .setTitle(this.queue[msg.guildId][0].name)
...
client.on('messageCreate', async (msg) => {
    ...
    if (msg.content === `${prefix}help`) {
        msg.channel.send({embeds:[new EmbedBuilder()
                ...
                .setTitle('Commands:')
                .setDescription(
                    `${prefix}p - play track by url\n` +
                    `${prefix}s - search track by keywords\n` +
